...
test('feedback form shows validation error for too-short message', async ({ page }) => {
  ...
  await Promise.all([
    ...
    page.getByRole('button', { name: /lähetä|send/i }).first().click(),
  ])
...
test('feedback form shows success state after valid submission', async ({ page }) => {
  await page.goto('/work/2')
  await page.getByText(/Jäikö jotain epäselväksi|Anything unclear/i).click()
  await page.getByRole('textbox', { name: /Viesti|Message/i }).fill('Tämä on testipalaute joka on tarpeeksi pitkä.')
  await Promise.all([
    page.waitForLoadState('networkidle'),
    page.getByRole('button', { name: /lähetä|send/i }).first().click(),
  ])
  await expect(page.getByText(/Kiitos palautteesta|Thank you for your feedback/i)).toBeVisible({ timeout: 15000 })
})
