...
test.describe("Admin courses", () => {
  ...
  test("admin can edit a course", async ({ page }) => {
    // Create
    await page.goto("/admin/courses/new");
    await page.getByLabel(/Kurssin nimi \(FI\)/i).fill("Muokkauskurssi FI");
    await page.getByLabel(/Course name \(EN\)/i).fill("Edit Course EN");
    ...
    // Click Edit
    const row = page.locator("tr", { hasText: "Muokkauskurssi FI" }).first();
    await row.getByRole("link", { name: /muokkaa/i }).click();
    await expect(page).toHaveURL(/\/admin\/courses\/\d+/);

    // Update course name
    const field = page.getByLabel(/Kurssin nimi \(FI\)/i);
    await field.clear();
    await field.fill("Päivitetty Kurssi FI");
    await page.getByRole("button", { name: "Tallenna" }).click();
    await expect(page).toHaveURL("/admin/courses");
    await expect(page.getByRole("cell", { name: "Päivitetty Kurssi FI" })).toBeVisible();

    // Cleanup
    const updatedRow = page.locator("tr", { hasText: "Päivitetty Kurssi FI" }).first();
