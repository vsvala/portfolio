...
test("/projects shows page heading", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: /Valitut projektit|Selected Projects/i })
  ).toBeVisible();
});
...
test.describe("Admin projects", () => {
  ...
  test("admin can edit a project", async ({ page }) => {
    // Create
    await page.goto("/admin/projects/new");
    await page.getByLabel(/Otsikko \(FI\)/i).fill("Muokkausprojekti FI");
    await page.getByLabel(/Title \(EN\)/i).fill("Edit Test Project EN");
    await page.getByRole("button", { name: "Tallenna" }).click();
    await expect(page).toHaveURL("/admin/projects");

    // Click Edit
    const row = page.locator("tr", { hasText: "Muokkausprojekti FI" }).first();
    await row.getByRole("link", { name: /muokkaa/i }).click();
    await expect(page).toHaveURL(/\/admin\/projects\/\d+/);

    // Update title
    const field = page.getByLabel(/Otsikko \(FI\)/i);
    await field.clear();
    await field.fill("Päivitetty Projekti FI");
    await page.getByRole("button", { name: "Tallenna" }).click();
    await expect(page).toHaveURL("/admin/projects");
    await expect(page.getByRole("cell", { name: "Päivitetty Projekti FI" })).toBeVisible();

    // Cleanup
    const updatedRow = page.locator("tr", { hasText: "Päivitetty Projekti FI" }).first();
