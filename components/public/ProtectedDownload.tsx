...
export function ProtectedDownload({ filename, labelFi, labelEn, lang }: Props) {
  ...
  async function handleDownload() {
    ...
    try {
      const res = await fetch(
        `/api/protected-doc?filename=${encodeURIComponent(filename)}&password=${encodeURIComponent(password)}`
      )
      if (res.status === 401) {
  ...
  return (
    <>
      ...
      <Dialog open={open} onClose={() => { setOpen(false); setPassword(''); setError('') }} maxWidth="xs" fullWidth>
        <DialogTitle>
          {lang === 'fi' ? 'Syötä salasana' : 'Enter password'}
        </DialogTitle>
        <DialogContent>
