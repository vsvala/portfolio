...
const introFi =
  "Ohjelmistosuunnittelija ja tietojenkäsittelytieteen maisteri Helsingin yliopistosta (2025). Yli 6 vuotta käytännön kokemusta Full Stack -kehityksestä Forecalla — React, TypeScript, Node.js ja Next.js. Poikkeuksellisen vahva visuaalinen silmä taideopettajataustani ansiosta.";
const introEn =
  "Software developer and M.Sc. in Computer Science from the University of Helsinki (2025). Over 6 years of hands-on full stack experience at Foreca — React, TypeScript, Node.js and Next.js. Strong eye for visual design from a background as an art teacher.";

const civicActivities = [
  {
    fi: "Akateemisen viiniseuran tasa-arvo- ja ympäristövastaava",
    en: "Academic Wine Society, equality and environmental officer",
    year: "2018",
  },
  {
    fi: "KOL:n hallituksen opiskelijajäsen",
    en: "KOL Board, student member",
    year: "2004",
  },
  {
    fi: "AIESEC, työharjoittelijan kummi",
    en: "AIESEC, trainee mentor",
    year: "1998–1999",
  },
  {
    fi: "Uusien opiskelijoiden tutorohjaaja",
    en: "Tutor for new students",
    year: "1998",
  },
  {
    fi: "Lukion oppilaskunnan sihteeri ja rahastonhoitaja",
    en: "Upper secondary school student council, secretary and treasurer",
    year: "1993–1995",
  },
];

const hobbyFi =
  "Harrastan öljyvärimaalausta ja monipuolisesti liikuntaa kuten tanssia, joogaa ja jumppia. Vahvuuksiani ovat sosiaalisuus, positiivisuus, luovuus, ideointikyky, organisointitaidot ja ohjaustaidot. Lisäksi olen utelias ja innokas oppimaan ja kokeilemaan aina uutta.";
const hobbyEn =
  "I paint with oils and keep active through dancing, yoga, and aerobics. My strengths include creativity, positivity, strong organisational and coaching skills — paired with a natural curiosity and a drive to keep learning.";
...
export default async function CvPage() {
  ...
  const skillsGrouped = SKILL_CATEGORY_KEYS.reduce<Record<string, string[]>>((acc, cat) => {
  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: "flex", gap: 3, alignItems: "flex-start", mb: 3 }}>
        ...
        <Box>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
            virva.svala(at)gmail.com · linkedin.com/in/virvasvala · github.com/vsvala ·
            virvasvala.com
          </Typography>
      ...
      <Box sx={{ mt: 2 }}>
        {SKILL_CATEGORY_KEYS.filter((cat) => skillsGrouped[cat]?.length > 0).map((cat) => (
      ...
      <Box sx={{ mt: 2 }}>
        {certifications.map((c) => (
          ...
          >
            <Box>
              <Typography variant="body2" sx={{ fontWeight: 600 }}>
                {isFi ? c.title_fi : c.title_en}
              </Typography>
              <Typography variant="body2" color="text.secondary">
                {isFi ? c.issuer_fi : c.issuer_en}
              </Typography>
            </Box>
            <Typography variant="body2" color="text.secondary" sx={{ whiteSpace: "nowrap" }}>
              {c.year}
