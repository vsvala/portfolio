  ...
  return (
    <Box sx={{ mb: 4 }}>
      {sections.map((section, i) => {
        ...
        const isHeader = first.length > 0 && first === first.toUpperCase() && !/^[•\-]/.test(first);
        if (isHeader) {
          return (
            <Box key={i} sx={{ mb: 2 }}>
        ...
        return (
          <Typography
            key={i}
            variant="body1"
            sx={{ lineHeight: 1.9, whiteSpace: "pre-line", mb: 2 }}
          >
            {section}
          </Typography>
        );
...
export default async function WorkDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <Container maxWidth="md" sx={{ py: 6 }}>
      <LinkButton href="/work" startIcon={<ArrowBackIcon />} sx={{ mb: 3 }} variant="text">
      ...
      <FeedbackForm
        ...
        targetTitle={
          lang === "fi"
            ? `${work.role_fi} — ${work.company_name_fi}`
            : `${work.role_en} — ${work.company_name_en}`
        }
      />
    </Container>
  );
